...
final readonly class Money extends AbstractMoney
{
     ...
     *
     * The monies must share the same currency.
    ...
    public function plus(AbstractMoney|BigNumber|int|string $that, RoundingMode $roundingMode = RoundingMode::Unnecessary): Money
    {
        ...
        if ($that instanceof Money) {
            if (! $this->context->isEqualTo($that->context)) {
                throw ContextMismatchException::contextMismatchWithHint($this->context, $that->context, __FUNCTION__);
            }
     ...
     *
     * If the operand is a RationalMoney, the context check is skipped and the result is fitted to this Money's context.
     * This can be used intentionally to perform cross-context arithmetic: `minus($money->toRational())`.
     *
     * The resulting Money has the same context as this Money. If the result needs rounding to fit this context, a
     * rounding mode can be provided. If a rounding mode is not provided and rounding is necessary, an exception is
     * thrown.
    ...
    public function allocate(array $ratios, AllocationMode $mode): array
    {
        ...
        $stepCounts = $mode->getStrategy()->allocate($amountInSteps, $ratios);
