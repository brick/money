...
class CachedProviderTest extends AbstractTestCase
{
    ...
    public function testSameCurrencyReturnsOneWithoutCallingWrappedProvider(): void
    {
        $mock = new ProviderMock();
        $provider = new CachedProvider($mock, new ArrayCache());
    ...
    public function testSameCurrencyReturnsOneWithDimensionsWithoutCallingWrappedProvider(): void
    {
        $mock = new ProviderMock();
        $provider = new CachedProvider($mock, new ArrayCache());
        ...
        self::assertBigNumberEquals(
            '1',
            $provider->getExchangeRate(Currency::of('EUR'), Currency::of('EUR'), ['date' => new DateTimeImmutable('2026-03-12')]),
        );
        self::assertSame(0, $mock->getCalls());
    }
    ...
    public function testTtlIsForwardedToCacheSet(): void
    {
        $eur = Currency::of('EUR');
        $usd = Currency::of('USD');

        foreach ([null, 60, new DateInterval('PT1H')] as $ttl) {
            $recordedTtl = 'not set';

            $cache = new class($recordedTtl) implements CacheInterface {
                private array $items = [];

                public function __construct(
                    private mixed &$recordedTtl,
                ) {
                }
            ...
            $provider = new CachedProvider(new ProviderMock(), $cache, ttl: $ttl);
            $provider->getExchangeRate($eur, $usd);

            self::assertSame($ttl, $recordedTtl);
        }
    }
